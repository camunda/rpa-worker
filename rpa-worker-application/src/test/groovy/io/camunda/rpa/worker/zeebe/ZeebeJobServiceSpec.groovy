...
class ZeebeJobServiceSpec extends Specification implements PublisherUtils {
	...
	void "Runs received task and reports Robot failure/error"(Result result, String expectedCode) {
		...
		then:
		1 * robotService.execute(script, [], [], _, null, _, _) >> Mono.just(new ExecutionResults([main: new ExecutionResults.ExecutionResult("main", result, "", expectedOutputVars, Duration.ZERO)],
				result, 
				expectedOutputVars, 
				Stub(Path), 
				Duration.ZERO))
